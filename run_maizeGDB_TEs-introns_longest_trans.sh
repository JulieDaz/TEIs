...
do
    ...
    if [ "$line" != "B73-v5" ] ; then
        ...
        bash ~/maize/pipeline_SVs-introns/maizeGDB_TEs-introns_longest_trans.sh $line
    fi
done
...
# python3 ~/maize/pipeline_SVs-introns/calculate_nb_exons_lengths.py -gff Zm-B73-REFERENCE-NAM-5.0_Zm00001eb.1.gff3

# ## get intron from longest transcript
# Rscript ~/maize/pipeline_SVs-introns/get_introns_longest_trans.R Zmays-B73-v5_genes-introns.bed Zm-B73-REFERENCE-NAM-5.0_Zm00001eb.1.gff3.exoncat Zmays-B73-v5_genes-introns_longest_trans.bed
# ## intersect between the SVinGene ovl and the introns
# bedtools intersect -a Zmays-B73-v5_MaizeGDB_allTEs_genes--intersect_ovl.bed -b Zmays-B73-v5_genes-introns_longest_trans.bed -wo > Zmays-B73-v5_MaizeGDB_allTEs_introns_longest_trans--intersect.txt

# # ## calculate perc of ovl, selection based on threshold
# python3 ~/maize/pipeline_SVs-introns/calculate_coverage_ovl.py -i Zmays-B73-v5_MaizeGDB_allTEs_introns_longest_trans--intersect.txt

# select unique ovl if several ovl in the same intron
python3 ~/maize/pipeline_SVs-introns/select_unique_ovl_TEfam.py -i  Zmays-B73-v5_MaizeGDB_allTEs_introns_longest_trans--intersect_ovl_coverage.txt

# ### Adds category of overlaps between introns and TE in the selected file
python3 ~/maize/pipeline_SVs-introns/add_cov_cat_selected_intron-TE_ovl.py -i Zmays-B73-v5_MaizeGDB_allTEs_introns_longest_trans--intersect_ovl_coverage_selected.txt
